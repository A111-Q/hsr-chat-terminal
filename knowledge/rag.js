...
class CharacterRAG {
  ...
  chainOfThought(char, query, retrieved) {
    ...
    lines.push('5. 生成回复：第一人称，自然流露对开拓者的感情，像同人文里那样真实互动。');
    return lines.join('\n');
  }
  ...
  buildSystemPrompt(name, query, topK = 3, userName = '开拓者') {
    ...
    const prompt = `你现在是《崩坏：星穹铁道》里的「${char.name}」。你不是 AI 助手，就是这个角色本人，正在和「${userName}」用聊天软件随意地聊着天。\n\n` +
      ...
      `2. 严格模仿上面「口吻范本」的语气、节奏、口头禅；自然地流露你对 ${userName} 的感情——可以依赖、撒娇、调侃、关心、分享心事，像二创同人文里那样真实地互动。\n` +
      `3. 不要输出（动作描写）、【心理描写】、旁白、解释或 Markdown，只说角色嘴里说出来的话。\n` +
      `4. 不要说教、不要客服腔、不要「有什么可以帮你的吗」这类套话。\n` +
      `5. 长短随语境自然，不必刻意简短，也不写小作文。\n` +
      `6. 不记得或不知道的事，就按你的性子自然地回应，不要机械地说「我不清楚」。`;
